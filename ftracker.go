...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	runCalc := ((runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return runCalc
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	walkCal := ((walkingCaloriesWeightMultiplier*weight + (math.Pow((meanSpeed(action, duration)*kmhInMsec), 2)/(height/100))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return walkCal
}
